  ...
  async #run_tasks(tgClients) {
    ...
    if (settings.USE_NON_THREAD) {
      ...
      if (settings.MAX_CONCURRENT_ACCOUNT > 10) {
        console.log("\n");
        logger.warning(
          `<ye>[tonstation]</ye> | MAX_CONCURRENT_ACCOUNT greater than 10 may cause 403 errors while running the bot\n`
        );
        ...
        return limit(async () => {
          ...
          try {
            if (num !== limit.activeCount) {
              ...
              logger.info(
                `<ye>[tonstation]</ye> | <gr>Starting thread</gr> <la>${
                  limit.activeCount
                }</la> of <pi>${
      ...
      logger.info(
        `<ye>[tonstation]</ye> | All threads completed | Sleeping ${sleep_thread} seconds before rerunning threads again`
      );
